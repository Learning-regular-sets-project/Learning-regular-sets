...
def menu():
    ...
    transitions_dic = {}
    transition_dic = {}
    print("Welcome(The states of the output are numbers.)")
    print("1) Learn a regular set")
    print("#) Exit")
    option = input("Enter a option: ")
    if option == "1":
        print("")
        Q = input(
            "Please put the states(separate by comas) of the dfa that recognizes the unkown lenguajes: "
        ).split(",")
        ...
        while state != "Break":
            ...
            if state in Q:
                transition_funtion_0 = input(
                    "Please put the state to which the state already entered passes when reading 0: "
                )
                transition_funtion_1 = input(
                    "Please put the state to which the state already entered passes when reading 1: "
                )
        ...
        if symmetric_difference(result, dfa) == None:
            print(
                "The machine found and learn the next regular lenguajes ",
                result.__dict__,
            )
            print("Thanks")
            menu()
